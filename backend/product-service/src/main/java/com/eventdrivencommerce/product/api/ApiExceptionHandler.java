...
public class ApiExceptionHandler {
    ...
    private static final String ERROR_BASE = "https://errors.event-driven-commerce.test/product/";

    @ExceptionHandler(ProductNotFoundException.class)
    ResponseEntity<ProblemDetail> notFound(ProductNotFoundException exception, HttpServletRequest request) {
        return response(HttpStatus.NOT_FOUND, "product-not-found", "Product not found", exception.getMessage(), request);
    }

    @ExceptionHandler(DuplicateSkuException.class)
    ResponseEntity<ProblemDetail> duplicateSku(DuplicateSkuException exception, HttpServletRequest request) {
        return response(HttpStatus.CONFLICT, "duplicate-sku", "SKU already exists", exception.getMessage(), request);
    }

    @ExceptionHandler(ConcurrentProductModificationException.class)
    ResponseEntity<ProblemDetail> concurrentChange(ConcurrentProductModificationException exception,
                                                    HttpServletRequest request) {
        return response(HttpStatus.CONFLICT, "concurrent-modification", "Concurrent product modification",
                exception.getMessage(), request);
    }

    @ExceptionHandler({IllegalArgumentException.class, IllegalStateException.class})
    ResponseEntity<ProblemDetail> semanticError(RuntimeException exception, HttpServletRequest request) {
        return response(HttpStatus.UNPROCESSABLE_ENTITY, "invalid-product", "Product data is not valid",
                exception.getMessage(), request);
    }
    ...
    ResponseEntity<ProblemDetail> unexpected(Exception exception, HttpServletRequest request) {
        LOGGER.error("Unhandled product service error", exception);
